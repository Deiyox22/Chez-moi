...
export async function searchLiveProducts({ query, maxPrice = null, styles = [], colors = [], stores = [], limit = 5 }) {
  ...
  const brief = [
    ...
    '',
    `Trouve au maximum ${limit} produits, puis appelle enregistrer_produits.`,
  ]
    .filter(Boolean)
    .join('\n');
  ...
  for (let iteration = 0; iteration < MAX_ITERATIONS; iteration += 1) {
    ...
    if (recorded) {
      const products = (recorded.input?.produits || []).slice(0, limit).map((product) => normalize(product, storesById));
      return products.filter((product) => product.url && product.title);
    }
